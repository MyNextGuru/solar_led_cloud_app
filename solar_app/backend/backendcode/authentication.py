...
def register_user():
    try:
        ...
        if role not in ['manufacturer', 'user']:
            return jsonify({'error': 'Invalid role. Must be "manufacturer" or "user".'}), 400
...
# ✅ 2. Register only in Firebase Auth (optional)
@app.route('/auth-only-register', methods=['POST'])
def auth_only_register():
    try:
        data = request.get_json()
        email = data.get('email')
        password = data.get('password')
        ...
        return jsonify({
            'message': 'User registered in Firebase Auth only',
            'uid': user.uid,
            'status': 'success'
        }), 200

    except Exception as e:
        print("❌ Exception during /auth-only-register:", str(e))
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
